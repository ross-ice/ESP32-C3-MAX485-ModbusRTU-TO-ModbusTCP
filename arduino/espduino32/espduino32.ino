...
#define RXD2 3        // DI ของ MAX485 (RX)
#define TXD2 0        // RO ของ MAX485 (TX)
#define REDE 26       // DE และ RE ควบคุมทิศทาง (GPIO)
#define LED_STATUS 2  // GPIO แสดงสถานะ WiFi (active low)
...
void updateLEDStatus() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    digitalWrite(LED_STATUS, HIGH);  // active HIGH = ON ติดค้าง
  }
}
...
void setup() {
  ...
  digitalWrite(LED_STATUS, LOW);  // LOW ปิด LED เริ่มต้น
